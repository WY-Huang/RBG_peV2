...
if __name__ == "__main__":
    ...
    # 1> 读取数据(1, c, h, w)
    batch_size = 36
    datapath = "pe_data/data_6762"
    trans = transforms.ToTensor()
    trainsets = PeDataSet(datapath, trans)
    train_dataloader = DataLoader(trainsets, batch_size, shuffle=True, num_workers=8, drop_last=True)
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_bar):
            ...
            train_bar.set_postfix_str(
                f"Epoch: {epoch}, Lr: {curr_lr}, Loss_mean: {np.mean(Loss)}, "
